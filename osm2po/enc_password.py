...
#
#   Based off of the sampel code TJEncryptPassword.py
#   Note:  Command line options have changed quite a bit.
...
def createPasswordEncryptionKeyFile (sTransformation, sAlgorithm, sMode, sPadding, nKeySizeInBits, sMatch, sMac, sPassKeyFileName):
    try:
        with open(os.path.expanduser(sPassKeyFileName), "x", encoding="iso-8859-1") as f:
        ...
        pass ##file already exists, no problem

    #in all cases (re) set the file to be user read only
    os.chmod (os.path.expanduser(sPassKeyFileName), 0o400)


def createEncryptedPasswordFile (sPassKeyFileName,   sEncPassFileName ):
    ...
    sPassword = getpass.getpass ("Enter password to encrypt:").encode().decode ('unicode_escape') # for backslash uXXXX escape sequences
    ...
    with open (os.path.expanduser(sEncPassFileName), "w", encoding="iso-8859-1") as f:
        #f.write ("# Teradata SQL Driver encrypted password file\n" +
        f.write ("# Encrypted password file\n" +
            "version=1\n" +
            "match=" + mapPassKey ["match"] + "\n" +
...
def loadPropertiesFile (sFileName):
    ...
    with open (os.path.expanduser(sFileName), encoding="iso-8859-1") as f:
...
def decryptPassword (sPassKeyFileName, sEncPassFileName):
    ...
    msg="{} specifies {} with {}-bit key and {}".format (sPassKeyFileName, sTransformation, nKeySizeInBits, sMACAlgorithm)
...
if __name__ == '__main__':
    ...
    p.add_argument("--keyfile", default='~/.tdpasswordkey', type=str, help="Secret key file" )
    p.add_argument("--passwordfile", default='~/.tdpassword', type=str, help="Password file")
    p.add_argument("--decrypt", default=False, action="store_true",  help="Quietly decrypt and cleanly return the password in passwordfile")
    args = p.parse_args()
    if not args.decrypt:
    ...
    sKeySizeInBits   = 256
    sMac             = 'HmacSHA256'
    sPassKeyFileName = args.keyfile
    sEncPassFileName = args.passwordfile
    ...
    if args.decrypt:
        print (decryptPassword (sPassKeyFileName, sEncPassFileName))
    else:
        createPasswordEncryptionKeyFile (sTransformation, sAlgorithm, sMode, sPadding, nKeySizeInBits, sMatch, sMac, sPassKeyFileName)
        createEncryptedPasswordFile (sPassKeyFileName, sEncPassFileName)
        pwd = decryptPassword (sPassKeyFileName, sEncPassFileName)
        msg = "[{}] encrypted by ENCRYPTED_PASSWORD(file:{},file:{})".format (pwd, sPassKeyFileName, sEncPassFileName)
        print (msg)
